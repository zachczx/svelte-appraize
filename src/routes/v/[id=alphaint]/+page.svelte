<script>
	...
	/* 	$effect(() => {
		console.log(countGradeA);
		console.log(countGradeB);
		console.log(countGradeC);
		console.log(countGradeD);
	}); */
...
<div class="grid grid-cols-5">
	<div id="sidebar" class="space-y-10 bg-base-100 pe-2 ps-10 pt-10 text-2xl">
		<div class="grid grid-cols-2 gap-2 rounded-xl border border-gray-300 bg-base-200 px-4 py-4">
			<h2 class="col-span-2">Summary</h2>
			<div class="card bg-base-100">
				<div class="grid h-full content-center justify-items-center p-1">
					<h3 class="text-3xl font-bold">A</h3>
					<p class="text-base">
						{#await data.streamed.result}
							<span class="loading loading-spinner justify-self-center text-primary"></span>
						{:then}
							{countGradeA}
						{/await}
					</p>
			...
			<div class="card bg-base-100">
				<div class="grid h-full content-center justify-items-center p-1">
					<h3 class="text-3xl font-bold">B</h3>
					<p class="text-base">
						{#await data.streamed.result}
							<span class="loading loading-spinner justify-self-center text-primary"></span>
						{:then}
			...
			<div class="card bg-base-100">
				<div class="grid h-full content-center justify-items-center p-1">
					<h3 class="text-3xl font-bold">C</h3>
					<p class="text-base">
						{#await data.streamed.result}
							<span class="loading loading-spinner justify-self-center text-primary"></span>
						{:then}
			...
			<div class="card bg-base-100">
				<div class="grid h-full content-center justify-items-center p-1">
					<h3 class="text-3xl font-bold">D</h3>
					<p class="text-base">
						{#await data.streamed.result}
							<span class="loading loading-spinner justify-self-center text-primary"></span>
						{:then}
							{#key countGradeD}{countGradeD}{/key}
						{/await}
					</p>
				</div>
			</div>
		...
		<div
			class="space-y-4 rounded-xl border border-gray-300 bg-base-200 px-4 py-4 shadow-lg shadow-gray-400"
		>
			<h2>Add New</h2>
			<form method="POST" action="?/insert" class="flex flex-wrap gap-1" use:enhance>
				<label class="input input-bordered input-primary flex w-full items-center text-lg">
					{#if helperText}
						<div
				...
				<button
					class="btn btn-primary relative w-full text-xl font-bold"
					onclick={() => {
					...
					/>
					<IconoirStar
						class="absolute -left-4 -top-1 h-[1.5em] w-[1.5em] overflow-visible stroke-pink-300 text-red-500 transition delay-300 duration-500 ease-out"
					/>
				{/if} -->
				</button>
				{#if helperText}
	...
	<div class="col-span-4 space-y-12 pt-10">
		<ol>
								...
								<div class="col-span-5 flex justify-end p-2 md:col-span-2">
									...
									<form method="POST" class="mx-3 self-center" action="?/delete" use:enhance>
										...
										<button
											...
											><Trash class="inline h-[1.5em] w-[1.5em] stroke-red-400" />
										</button>
										{#if helperText}
											<div
												class="tooltip tooltip-top tooltip-open tooltip-secondary font-bold"
